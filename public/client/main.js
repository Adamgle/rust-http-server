...
function deserializeCookies() {
    return Object.fromEntries(document.cookie.split(";").map((c) => c.trim().split("=")));
}
...
function handleError(error) {
    const errorModal = ElementCreator.createElement("div", {
        class: "error-modal",
        style: "position: fixed; top: 10px; left: 10px; color: #fff; background: #111; border: 1px solid #f44336; padding: 1rem; z-index: 1000; max-width: 300px; box-shadow: 0 2px 5px rgba(0,0,0,0.2); border-radius: 4px;",
    });
    const errorTitle = ElementCreator.createElement("h4", {
        style: "margin: 0 0 5px 0; color: white;",
    }, "Error");
    const errorContent = ElementCreator.createElement("pre", {
        style: "margin: 0; white-space: pre-wrap; font-size: 16px; max-height: 200px; overflow-y: auto;",
    }, JSON.stringify(error, null, 2));
    const closeButton = ElementCreator.createElement("button", {
        style: "position: absolute; top: 5px; right: 5px; background: none; border: none; cursor: pointer; font-weight: bold; color: #f44336; font-size: 20px;",
    }, "×");
...
APIConfig.SERVER_ROOT = "http://localhost:5000";
APIConfig.DATABASE_ROOT = "database";
// For backward compatibility
const { SERVER_ROOT, DATABASE_ROOT, buildUrl, DATABASE_URL, DATABASE_TASKS_PATH, DATABASE_USERS_PATH, API_USER_INFO, } = {
...
class Components extends ElementCreator {
    static async UserInfo() {
        ...
        if (user) {
            ...
            const signOutButton = ElementCreator.createElement("button", {
                id: "sign-out-button",
            }, "Sign Out");
...
class API {
    ...
    static async getTasks() {
        ...
        try {
            ...
            if (user) {
                try {
                    ...
                    Object.values(data).forEach(({ value, id }) => {
                        ...
                        const task = ElementCreator.createElement("div", { class: "task", id }, value);
                        const deleteButton = ElementCreator.createElement("button", { class: "delete-button" }, "Delete");
            ...
            console.log(`User not logged in, skipping task loading: ${error.message}`);
        }
        return null;
    }
    static async registerAddUser() {
        ...
        loginForm.appendChild(ElementCreator.createElement("input", {
            type: "text",
            name: "email",
            placeholder: "email",
            required: "true",
        ...
        registerForm.appendChild(ElementCreator.createElement("input", {
            type: "text",
            name: "email",
            placeholder: "email",
            required: "true",
        ...
        registerForm.appendChild(ElementCreator.createElement("button", {}, "Register"));
        ...
        try {
            taskForm.addEventListener("submit", async (e) => {
                ...
                try {
                    ...
                    const data = await res.json();
                    const task = ElementCreator.createElement("div", { class: "task", data: data.id }, value);
                    taskContainer.appendChild(task);
                    taskContainer.appendChild(deleteButton);
                    tasks.appendChild(taskContainer);
